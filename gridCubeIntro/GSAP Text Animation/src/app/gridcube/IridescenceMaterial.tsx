...
export function Iridescence({
  ...
  const dirValue = useMemo(() => new Vector3(...direction).normalize(), [direction])
  const fresColor = useMemo(() => new Color(fresnelColor), [fresnelColor])
  const emissiveColorValue = useMemo(() => new Color(emissiveColor), [emissiveColor])

  return (
    // @ts-expect-error - Custom material name provided via extend()
    <iridescenceMaterial
      ref={materialRef}
      uColorA={colorValueA}
      uColorB={colorValueB}
