...
// Hook version that can access renderer for optimal anisotropy
export function useSharedTexturesWithRenderer(imageUrls) {
  const { gl } = useThree()
  const fallbackTextures = useMemo(
    () => imageUrls.map((_, index) => createFallbackTexture(index)),
    [imageUrls]
      ...
      imageUrls.map(
        (url) =>
          new Promise((resolve) => {
            loader.load(
              url,
              (texture) => resolve(prepareTexture(texture, gl)),
              undefined,
              () => resolve(null)
            )
          })
